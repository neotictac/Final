...
var base = Rebase.createClass({
  apiKey: "AIzaSyBuV3mCFYU9cg4oqKt7SCXVFZWslDlV8RE",   // replace with your Firebase application's API key
  databaseURL: "https://buyflix-final-ca433.firebaseio.com/", // replace with your Firebase application's database URL
})
...
var App = React.createClass({
  ...
  viewChanged: function(view) {
    ...
    if (this.state.currentView === 'latest') {
      return (
        view: movieData.sort(this.movieCompareByReleased),
      )
    }
    if (this.state.currentView === 'alpha') {
      return (
        var view = movieData.sort(this.moviewCompareByTitle)
      )
    this.setState({
      currentView: view
    })
  ...
  render: function() {
    return (
      <div>
        ...
        <SortBar bold={this.state.bold} movieCount={this.state.movies.length} viewChanged={this.viewChanged} />
